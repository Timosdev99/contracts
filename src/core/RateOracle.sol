...
contract RateOracle is IRateOracle, AccessControl {
    ...
    function updateRate(string calldata currency, uint256 rate, uint256 confidence)
        external
        onlyRole(ORACLE_UPDATER_ROLE)
    {
        require(confidence <= 100, "Invalid confidence");

        rates[currency][msg.sender] = Rate({value: rate, timestamp: block.timestamp, confidence: confidence});
